...
class zigbee2Mqtt_Bridge(HABApp.Rule):
   ...
   def __init__(self):
      ...
      self.mqttitems = {}
      self.mqttitems['BewegungsMelder'] = HABApp.DictParameter( parmFile, 'MotionDetectors', default_value=None)
      self.mqttitems['Licht']           = HABApp.DictParameter( parmFile, 'Lights',          default_value=None)
      self.mqttitems['FernBedienung']   = HABApp.DictParameter( parmFile, 'RemoteControls',  default_value=None)
      ...
      self.mqtt_permitToJoin_request_item  = MqttItem.get_create_item(self.mqtt_base_request_topic  + "permit_join")
      ## MQTT item read requestToJoin response
      self.mqtt_permitToJoin_response_item = MqttItem.get_create_item(self.mqtt_base_response_topic + "permit_join")
      ## OpenHAB item to trigger permitToJoin
      self.oh_permitToJoin_State      = SwitchItem.get_item('Zigbee2Mqtt_PermitJoin')
      ## OpenHAB item for permitToJoin timeout
      self.oh_permitToJoin_Timeout    = NumberItem.get_item('Zigbee2Mqtt_PermitJoin_Timeout')
      ## OpenHAB item to store remaining permitToJoin time
      self.oh_permitToJoin_TimeRemain = NumberItem.get_item('Zigbee2Mqtt_PermitJoin_TimeRemain')

      ## MQTT item trigger healthcheck
      self.mqtt_healthCheck_request_item  = MqttItem.get_create_item(self.mqtt_base_request_topic  + "health_check")
      ## MQTT item to read healthcheck response
      self.mqtt_healthCheck_response_item = MqttItem.get_create_item(self.mqtt_base_response_topic + "health_check")
      ...
      self.oh_networkMap_request_State   = SwitchItem.get_item('Zigbee2Mqtt_NetworkMap')
      ## MQTT item trigger creation of network map
      self.mqtt_networkMap_request_item  = MqttItem.get_create_item(self.mqtt_base_request_topic  + "networkmap")
      ## MQTT item to read network map response
      self.mqtt_networkMap_response_item = MqttItem.get_create_item(self.mqtt_base_response_topic + "networkmap")
      ## OpenHAB item to save the date of the last network map check 
      self.mqtt_networkMap_update        = DatetimeItem.get_item('Zigbee2Mqtt_NetworkMap_Update')
      ...
      self.mqtt_TriggerUpdate_request_item  = MqttItem.get_create_item(self.mqtt_base_request_topic  + "device/ota_update/update")

      ## MQTT item to trigger bridge restart
      self.mqtt_BridgeRestart_request_item  = MqttItem.get_create_item(self.mqtt_base_request_topic  + "restart")

      ## OpenHAB item to trigger the bridge restart
      self.oh_RestartBridge_State = SwitchItem.get_item('Zigbee2Mqtt_Restart_Bridge')
    
      # event registration
      self.listen_event(self.mqtt_base_info,                           self.info_topic_updated,         ValueUpdateEventFilter())

      self.listen_event(self.mqtt_base_response_topic + "permit_join", self.permitToJoin_topic_updated, ValueUpdateEventFilter())
      self.oh_permitToJoin_State.listen_event(self.on_PermitJoin, ItemStateChangedEventFilter())

      self.listen_event(self.mqtt_base_response_topic + "health_check", self.healthCheck_topic_updated, ValueUpdateEventFilter())

      self.listen_event(self.mqtt_base_response_topic + "networkmap",   self.networkmap_topic_updated, ValueUpdateEventFilter())

      self.oh_networkMap_request_State.listen_event(self.on_RequestNetworkMap, ItemStateChangedEventFilter())

      self.oh_RestartBridge_State.listen_event(self.on_BridgeRestartRequest, ItemStateChangedEventFilter())
         ...
         for items in self.mqttitems[types].values():
            ...
            if not self.openhab.item_exists(triggerUpdateName):
                if self.openhab.create_item(item_type = "Switch", name = triggerUpdateName, label = triggerUpdateName, category = "switch", groups = ["Zigbee2Mqtt"]):
                    log.info( f"created new item {triggerUpdateName}. Waiting till item is in registry")
                    waitCounter = 0
                    while (not self.openhab.item_exists(triggerUpdateName)) & (waitCounter < 5):
                            log.info( f"waiting {waitCounter} seconds")
                            time.sleep(1)
                            
            (SwitchItem.get_item(triggerUpdateName)).listen_event(self.on_TriggerUpdate, ItemStateChangedEventFilter())
            log.info( f"register listener for {triggerUpdateName} item # {itemcount}")
   ...
   def on_PermitJoin(self, event):
      ...
      if event.value == "ON":
         ...
         if self.item_exists("Zigbee2Mqtt_PermitJoin_Timeout"):
            timeout = NumberItem.get_item("Zigbee2Mqtt_PermitJoin_Timeout").get_value()
   ...
   def healthCheck_topic_updated(self, event):
      ...
      if "data" in event.value:
        if "healthy" in event.value["data"]:
            self.oh_healthCheck_State.oh_send_command(self.stateMap[str(event.value["data"]["healthy"])])
...
##############
   ...
   def trigger_networkmap_update(self):
      ...
      self.mqtt_networkMap_request_item.publish("{\"type\": \"graphviz\", \"routes\": false}")
   ...
   def on_TriggerUpdate(self, event):
      ...
      if event.value == "ON":
         ...
         if (deviceName.find("Licht") != -1):
            publish_value = publish_value + "Lampe/" + deviceName.replace("Licht", "")
         if (deviceName.find("FernBedienung") != -1):
            publish_value = publish_value + "FernBedienung/" + deviceName.replace("FernBedienung", "")
         if (deviceName.find("BewegungsMelder") != -1):
            publish_value = publish_value + "BewegungsMelder/" + deviceName.replace("BewegungsMelder", "")

         log.info( f"trigger update for {deviceName}")
         self.mqtt_TriggerUpdate_request_item.publish("{\"id\": \"" + publish_value + "\"}")
      else:
         publish_value = "{\"value\": false}"
