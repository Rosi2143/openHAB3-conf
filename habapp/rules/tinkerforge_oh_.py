...
class MyTinkerforgeRule(HABApp.Rule):
    ...
    def __init__(self):
        ...
        #        try:
        with open(
            os.path.join(scriptpath, "tinkerforge_oh.json"), "rt", encoding="utf8"
        ) as json_file:
            json_config = json.load(json_file)

        log.info("\n\n")
        log.info(
            "started at: %s", datetime.datetime.now().strftime("%d.%b %Y %H:%M:%S")
        )
        ...
        ipcon.connect(
            json_config["connection"]["host"], int(json_config["connection"]["port"])
        )  # Connect to brickd

        devices = {}
        for device in json_config["devices"]:
            log.info(
                "Found device %s in config of type %s", device["uid"], device["type"]
            )
            if device["type"] == "BrickletIo16V2":
                ...
                if "checkinterval_s" in device:
                    ...
                    log.info(
                        "add port check for %s at %s",
                        str(device["uid"]),
                        new_device.check_ports,
                    )
                    self.run.every(
                        start_time=datetime.timedelta(seconds=checkinterval_s),
