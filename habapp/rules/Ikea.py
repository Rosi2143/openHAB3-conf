...
class IkeaZigbeeDevices(HABApp.Rule):
    ...
    def __init__(self):
        ...
        self.min_battery_charge = HABApp.Parameter( parmFile, 'min_battery_charge', default_value=20)
        ## read the motion detectors from the parameter file
        self.MotionDetectors    = HABApp.DictParameter( parmFile, 'MotionDetectors', default_value=None)
        ## read the lights from the parameter file
        self.Lights             = HABApp.DictParameter( parmFile, 'Lights', default_value=None)
        ## read the remote controls from the parameter file
        self.RemoteControls = HABApp.DictParameter( parmFile, 'RemoteControls', default_value=None)
        ...
        for MotionDetector in self.MotionDetectors.values():
            mqtt_MotionDetector_topic = self.mqtt_base_topic + 'BewegungsMelder/' + MotionDetector
            self.listen_event(mqtt_MotionDetector_topic, self.motion_detect_updated, ValueUpdateEventFilter())
        ...
        for RemoteControl in self.RemoteControls.values():
            ...
            self.listen_event(mqtt_RemoteControl_topic, self.remote_control_updated, ValueUpdateEventFilter())
...
#########
    def OH_SwitchItem_Changed(self, oh_switchItem, new_value):
        ...
        if (state == "None") or (state == None):
            ...
            if ((state=="ON" and new_value == "OFF") or (state=="OFF" and new_value == "ON")):
                ret = True
        logger.debug( f"OH_SwitchItem_Changed for {oh_switchItem.name} {ret} <-- {oh_switchItem.get_value()} vs. {new_value}")
        return ret
    ...
    def OH_DimmerItem_Changed(self, oh_dimmerItem, new_value):
        ...
        if value == "None":
            ...
            if ( abs(value - new_value) > 5):
                ret = True
        logger.debug( f"OH_DimmerItem_Changed for {oh_dimmerItem.name} {ret} <-- {value} vs. {new_value}")
        return ret
    ...
    def OH_NumberItem_Changed(self, oh_numberItem, new_value):
        ...
        if value == "None":
            ...
            if (value != new_value):
                ret = True
        logger.debug( f"OH_NumberItem_Changed for {oh_numberItem.name} {ret} <-- {value} vs. {new_value}")
        return ret
    ...
    def switchitem_update(self, event):
        ...
        mqttTopic = self.mqtt_base_topic + "Lampe/" + self.extractMqttItemName(event.name) + "/set"
    ...
    def dimmeritem_update(self, event):
        ...
        if "_Dimmer" in event.name:
            ...
            if "_ColorTemp" in event.name:
                ...
                logger.error("wrong callback dimmeritem_update for " + event.name)
                return

        mqttTopicValue = mqttTopicValue + str(mqttValue)

        if self.openhab.item_exists("Licht" + mqttName + "_Transition"):
            transitionItemValue= NumberItem.get_item("Licht" + mqttName + "_Transition").get_value(5)

        if transitionItemValue != 0:
            mqttTopicValue = mqttTopicValue + ", \"transition\": " + str(int(transitionItemValue))
    ...
    def numberitem_update(self, event):
        ...
        if "_Dimmer" in event.name:
            ...
            if "_ColorTemp" in event.name:
                ...
                logger.error("wrong callback numberitem_update for " + event.name)
                return
    ...
    def stringitem_update(self, event):
        ...
        if "_ColorTemp_String" in event.name:
            ...
            if "_Effect" in event.name:
                ...
                logger.error("wrong callback stringitem_update for " + event.name)
                return
...
###############################################################################
    ...
    def init_light(self): 
        ...
        for Light in self.Lights.values():
            ...
            if self.openhab.item_exists(dimmerItemName):
                ...
                dimmerItem.listen_event(self.dimmeritem_update, ItemStateEventFilter())
            else:
            ...
            if self.openhab.item_exists(dimmerMoveItemName):
                ...
                dimmerMoveItem.listen_event(self.numberitem_update, ItemStateEventFilter())
            else:
            ...
            if self.openhab.item_exists(dimmerStepItemName):
                ...
                dimmerStepItem.listen_event(self.numberitem_update, ItemStateEventFilter())
            else:
            ...
            if self.openhab.item_exists(colorTempItemName):
                ...
                colorTempItem.listen_event(self.dimmeritem_update, ItemStateEventFilter())
            else:
            ...
            if self.openhab.item_exists(colorTempStringItemName):
                colorTempStringItem = StringItem.get_item(colorTempStringItemName)
                colorTempStringItem.listen_event(self.stringitem_update, ItemStateEventFilter())
            else:
            ...
            if self.openhab.item_exists(colorTempMoveItemName):
                ...
                colorTempMoveItem.listen_event(self.numberitem_update, ItemStateEventFilter())
            else:
            ...
            if self.openhab.item_exists(colorTempStepItemName):
                ...
                colorTempStepItem.listen_event(self.numberitem_update, ItemStateEventFilter())
            else:
            ...
            if self.openhab.item_exists(effectItemName):
                ...
                effectItem.listen_event(self.stringitem_update, ItemStateEventFilter())
            else:
            ...
            if self.openhab.item_exists(transitionItemName):
                ...
                transitionItem.listen_event(self.numberitem_update, ItemStateEventFilter())
            else:
                logger.error(f"{transitionItemName} does not exist")

            mqtt_Light_topic = self.mqtt_base_topic + 'Lampe/' + Light
            self.listen_event(mqtt_Light_topic, self.light_updated, ValueUpdateEventFilter())
    ...
    def light_updated(self, event):
        ...
        logger.info( f"mqtt topic " + event.name + " updated to " + str(event.value))

        if "state" in event.value:
            stateItemName = "Licht"+ self.extractOhItemName(event.name) +"_State"
            if self.openhab.item_exists(stateItemName):
                switchItem = SwitchItem.get_item(stateItemName)
        ...
        if "brightness" in event.value:
            dimmerItemName = "Licht"+ self.extractOhItemName(event.name) +"_Dimmer"
            if self.openhab.item_exists(dimmerItemName):
                dimmerItem = DimmerItem.get_item(dimmerItemName)
        ...
        if "color_temp" in event.value:
            colorTempItemName = "Licht"+ self.extractOhItemName(event.name) +"_ColorTemp"
            if self.openhab.item_exists(colorTempItemName):
                colorTempItem = DimmerItem.get_item(colorTempItemName)
        ...
        if "update" in event.value:
            if "state" in event.value["update"]:
                UpdateItemName = "Licht"+ self.extractOhItemName(event.name) +"_UpdatePending"
...
###############################################################################
    ...
    def motion_detect_updated(self, event):
        ...
        logger.debug( f"mqtt topic " + event.name + " updated to\n" + json.dumps(event.value, indent=2))
        ...
        if self.openhab.item_exists(topic_items[2]+topic_items[3]+"_BatteryStatus"):
            my_oh_item_batterystate      = SwitchItem.get_item(topic_items[2]+topic_items[3]+"_BatteryStatus")
        else:
            logger.error( f"item " + topic_items[2]+topic_items[3]+"_BatteryStatus does not exist")
            exitcode = 1

        if self.openhab.item_exists(topic_items[2]+topic_items[3]+"_ChargingLevel"):
            my_oh_item_chargingLevel     = NumberItem.get_item(topic_items[2]+topic_items[3]+"_ChargingLevel")
        else:
            logger.error( f"item " + topic_items[2]+topic_items[3]+"_ChargingLevel does not exist")
            exitcode = 2

        if self.openhab.item_exists(topic_items[2]+topic_items[3]+"_UpdatePending"):
            my_oh_item_updatepending     = SwitchItem.get_item(topic_items[2]+topic_items[3]+"_UpdatePending")
        else:
            logger.error( f"item " + topic_items[2]+topic_items[3]+"_UpdatePending does not exist")
            exitcode = 3

        if self.openhab.item_exists(topic_items[2]+topic_items[3]+"_MotionState"):
            my_oh_item_motionstate       = SwitchItem.get_item(topic_items[2]+topic_items[3]+"_MotionState")
        else:
            logger.error( f"item " + topic_items[2]+topic_items[3]+"_MotionState does not exist")
            exitcode = 4

        if self.openhab.item_exists(topic_items[2]+topic_items[3]+"_MotionDetectState"):
            my_oh_item_motiondetectstate = SwitchItem.get_item(topic_items[2]+topic_items[3]+"_MotionDetectState")
        else:
            logger.error( f"item " + topic_items[2]+topic_items[3]+"_MotionDetectState does not exist")
            exitcode = 5
        ...
        my_oh_item_batterystate.oh_send_command(self.stateMap[str(batteryWeak)])
        my_oh_item_updatepending.oh_send_command(self.stateMap[str(updateAvailable)])
        my_oh_item_motionstate.oh_send_command(self.stateMap[str(occupancy)])
        my_oh_item_motiondetectstate.oh_send_command(self.stateMap[str(illuminanceAboveThreshold)])
        ...
        for key, item in keypress_items.items():
            ...
            if action == key:
                ...
                if self.openhab.item_exists(key_itemname):
                    ...
                    logger.debug( f"found action for {action} for {key}  :: {key_itemname} --> {item[1]}")
                ...
                logger.info(f"skip OH item {key_itemname} - for action {action}")
