...
class Zigbee2MqttBridge(HABApp.Rule):
    ...
    def __init__(self):
        ...
        self.state_map = {"FALSE": "OFF", "TRUE": "ON", "ON": "true", "OFF": "false"}
        ...
        self.oh_permit_to_join_state = SwitchItem.get_item("Zigbee2Mqtt_PermitJoin")
        log.info(
            "Zigbee2Mqtt_PermitJoin state: %s", self.oh_permit_to_join_state.get_value()
        )
        ...
        log.info(
            "Zigbee2Mqtt_PermitJoin_Timeout state: %s",
        ...
        self.oh_restart_bridge_state = SwitchItem.get_item("Zigbee2Mqtt_Restart_Bridge")

        # event registration
        self.listen_event(
            self.mqtt_base_info, self.info_topic_updated, ValueChangeEventFilter()
        )
    ...
    def info_topic_updated(self, event):
        ...
        log.info(
            "mqtt topic %s updated to %s (type = %s)",
            event.name,
            event.value,
            type(event),
        )
        ...
        if permit_join == "ON":
            if "permit_join_timeout" in event.value:
                ...
                if (remaining_time % 5) == 0:
                    self.oh_permit_to_join_time_remain.oh_send_command(remaining_time)
        else:
    ...
    def on_permit_join(self, event):
        ...
        log.info(
            "set %s updated to %s (type = %s)", event.name, event.value, type(event)
        )
    ...
    def on_trigger_update(self, event: ValueUpdateEvent):
        ...
        if event.value == "ON":
            ...
            if device_name.find("Licht") != -1:
                publish_value = (
                    publish_value + "Lampe/" + device_name.replace("Licht", "")
                )
