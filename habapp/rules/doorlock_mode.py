...
class DoorLockStatemachineRule(HABApp.Rule):
    ...
    def __init__(self):
        ...
        # CONFIG_PENDING
        config_pending_item = SwitchItem.get_item(thing_name + "_ConfigPending")
        ...
        config_pending_item.listen_event(self.error_changes, ValueChangeEventFilter())

        # ERROR_JAMMED
        error_jammed_item = SwitchItem.get_item(thing_name + "_ErrorJammed")
        ...
        DOOR_LOCK_STATE_MACHINE.send("tr_error_change")
        error_jammed_item.listen_event(self.error_changes, ValueChangeEventFilter())

        # UNREACHABLE
        unreachable_item = SwitchItem.get_item(thing_name + "_Unreachable")
        ...
        DOOR_LOCK_STATE_MACHINE.send("tr_error_change")
        unreachable_item.listen_event(self.error_changes, ValueChangeEventFilter())

        # REPORTED_LOCK_STATE
        reported_lock_item = StringItem.get_item(thing_name + "_LockState")
        ...
        # DarkOutside
        dark_outside_item = StringItem.get_item("Sonnendaten_Sonnenphase")
        ...
        presence_item.listen_event(self.presence_changed, ValueChangeEventFilter())

        # DoorState
        door_open_item = ContactItem.get_item("TuerWaschkuecheTerrasse_OpenState")
        ...
        # TerraceLight
        terrace_light_item = SwitchItem.get_item("LichtTerrasseUnten_State")
        ...
        DOOR_LOCK_STATE_MACHINE.send("tr_error_change")
        terrace_light_item.listen_event(self.light_changed, ValueChangeEventFilter())
    ...
    def dark_outside_changes(self, event: ValueChangeEvent):
        ...
        DOOR_LOCK_STATE_MACHINE.set_dark_outside(self.is_dark_outside(str(event.value)))
        DOOR_LOCK_STATE_MACHINE.send("tr_dark_outside_change")
        self.set_mode_item(DOOR_LOCK_STATE_MACHINE.get_state_name())
    ...
    def error_changes(self, event: ValueChangeEvent):
        ...
        DOOR_LOCK_STATE_MACHINE.set_lock_error(error_type, str(event.value) == "ON")
        DOOR_LOCK_STATE_MACHINE.send("tr_error_change")
        self.set_mode_item(DOOR_LOCK_STATE_MACHINE.get_state_name())
