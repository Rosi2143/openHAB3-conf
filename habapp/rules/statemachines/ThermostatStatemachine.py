...
# log:set INFO jsr223.jython.ThermostatStatemachine_create
...
def get_state_machine(item_name, logger):
    ...
    if thing_name not in thermostate_list:
        ...
        if platform.system() != "Windows":
            ...
            graphic_file_name=os.path.join(imagepath,
                                           thermostate_list[thing_name].get_name() + \
                                           "_thermostate_sm.png")
            graph().write_png(graphic_file_name)
    else:
...
def get_internal_state_machine_state(state_machine):
    ...
    info += "\n   _boost       = " + str(state_machine.cond_boost_is_active())
    info += "\n   _config      = " + str(state_machine.cond_config_is_active())
    info += "\n   _mode        = " + str(state_machine.get_mode())
    info += "\n   _party       = " + str(state_machine.cond_party_is_active())
    info += "\n   _window_open = " + str(state_machine.cond_window_is_open())
    return info
