...
class Mp3PlayerStatemachineRule(HABApp.Rule):
    ...
    def __init__(self):
        ...
        log.info(
            "##############################\nhandling DoorOpen: (%s)",
            str(outer_door_state),
        )
        MP3_PLAYER_STATE_MACHINE.set_outer_door_open((outer_door_state == "OPEN"))
        MP3_PLAYER_STATE_MACHINE.send("tr_outer_door_open")
        outer_door_item.listen_event(self.outer_door_open, ValueChangeEventFilter())

        # Window_Open
        window_open_item = GroupItem.get_item("gFenster")
        window_open_state = window_open_item.get_value()
        log.info(
            "##############################\nhandling open Windows: (%s)",
            str(window_open_state),
        )
        MP3_PLAYER_STATE_MACHINE.set_window_open((window_open_state == "OPEN"))
        MP3_PLAYER_STATE_MACHINE.send("tr_window_open")
        window_open_item.listen_event(self.window_open, ValueChangeEventFilter())

        self.set_mode_item(
            MP3_PLAYER_STATE_MACHINE.get_state_name(),
    ...
    def set_mode_item(self, state, level):
        ...
        if timeout_sec != 0:
            ...
            self.run.at(time=timedelta(seconds=timeout_sec), callback=self.timeout)
    ...
    def outer_door_open(self, event: ValueChangeEvent):
        ...
        MP3_PLAYER_STATE_MACHINE.set_outer_door_open(str(event.value) == "OPEN")
        MP3_PLAYER_STATE_MACHINE.send("tr_outer_door_open")
        self.set_mode_item(
            MP3_PLAYER_STATE_MACHINE.get_state_name(),
            MP3_PLAYER_STATE_MACHINE.get_light_level(),
    ...
    def window_open(self, event: ValueChangeEvent):
        ...
        MP3_PLAYER_STATE_MACHINE.set_window_open(str(event.value) == "OPEN")
        MP3_PLAYER_STATE_MACHINE.send("tr_window_open")
        self.set_mode_item(
            MP3_PLAYER_STATE_MACHINE.get_state_name(),
            MP3_PLAYER_STATE_MACHINE.get_light_level(),
