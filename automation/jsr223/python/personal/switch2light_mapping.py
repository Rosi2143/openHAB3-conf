...
def switch2wall_mapping_long(event):
    ...
    """
    switch2wall_mapping_long.log.info("rule fired because of %s", event.itemName)
    ...
    if switch_equipment_name in json_config["Long"]:
        ...
        if not isinstance(light_items, list):
            ...
            for light_item in light_items:
                ...
                switch2wall_mapping_short.log.info("setting item %s", light_item_name)
...
def switch2wall_mapping_short(event):
    ...
    """
    switch2wall_mapping_short.log.info("rule fired because of %s", event.itemName)
